...
written that will allow the channels to be put back to their correct
state after a re-scan.
...
objects, taking care of naming inconsistencies, etc. Currently on
...
the first record (obviously this will most likely fail, unless you
happen to have the ABC):
...
mythtv_cli_extensions currently requires Python 3.4.0 or later. It will
probably work on 3.3, but as I haven't been able to test it the check is
for 3.4.0. If a developer would like to confirm that it runs under 3.3
...
The automated tests also use fuzzywuzzy, which has better performance
